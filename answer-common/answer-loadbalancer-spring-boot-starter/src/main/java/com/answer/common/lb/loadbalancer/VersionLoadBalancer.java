 ...
 * @package com.answer.common.lb.loadbalancer
 * @className com.answer.common.lb.loadbalancer.VersionLoadBalancer
 */
package com.answer.common.lb.loadbalancer;

import com.answer.common.constant.CommonConstant;
import com.answer.common.lb.chooser.IRuleChooser;
import com.answer.common.lb.utils.QueryUtils;
...
public class VersionLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    private String getVersionFromRequestData(RequestData requestData){
        ...
        if(MapUtils.isNotEmpty(queryMap)&& queryMap.containsKey(CommonConstant.ANSWER_VERSION)&& StringUtils.isNotBlank(queryMap.get(CommonConstant.ANSWER_VERSION))){
            return queryMap.get(CommonConstant.ANSWER_VERSION);
        }else if(requestData.getHeaders().containsKey(CommonConstant.ANSWER_VERSION)){
            return requestData.getHeaders().get(CommonConstant.ANSWER_VERSION).get(0);
        }
        return null;
    }
    /**
