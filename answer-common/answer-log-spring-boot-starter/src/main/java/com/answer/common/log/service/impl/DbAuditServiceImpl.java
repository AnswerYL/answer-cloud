 ...
 * @className com.answer.log.service.impl.DbAuditServiceImpl
 */
 ...
 * @author answer_wx
 * @date 2024/1/19 11:24
 * @version 1.0
 */
@Slf4j
@Service
@ConditionalOnClass(JdbcTemplate.class)
@ConditionalOnProperty(name = "answer.audit-log.log-type", havingValue = "db")
public class DbAuditServiceImpl implements IAuditService {
    ...
    public DbAuditServiceImpl(@Autowired(required = false) LogDBProperties properties, DataSource dataSource) {
        //优先使用配置的日志数据源，否则使用默认的数据源
        if (properties != null && StringUtils.isNotEmpty(properties.getJdbcUrl())) {
            dataSource = new HikariDataSource(properties);
        }
